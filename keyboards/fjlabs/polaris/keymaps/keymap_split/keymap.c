...
enum custom_keycodes {
    ...
    MACRO_TOGGLE_TERMINAL,        // Macro para abrir terminal
    MACRO_SPOTIFY,
    MACRO_ENV
};
    ...
    switch (keycode) {
        case MACRO_DISCORD:
            if (record->event.pressed) {
                SEND_STRING(SS_DOWN(X_LALT) SS_TAP(X_SPACE) SS_UP(X_LALT) "discord" SS_TAP(X_ENTER));
            }
            break;
        case MACRO_TOGGLE_TERMINAL:
            if (record->event.pressed) {
                SEND_STRING(SS_DOWN(X_LCTL) SS_DOWN(X_LSFT) SS_TAP(X_GRV) SS_UP(X_LSFT) SS_UP(X_LCTL));
            }
            break;
        case MACRO_SPOTIFY:
            if (record->event.pressed) {
                SEND_STRING(SS_DOWN(X_LALT) SS_TAP(X_SPACE) SS_UP(X_LALT) "spotify" SS_TAP(X_ENTER));
            }
            break;
        case MACRO_ENV:
            if (record->event.pressed) {
                SEND_STRING("env/scripts/activate");
            }
            break;
    }
    return true;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_LAYER1] = LAYOUT_all( /* Fn_space */
        ...
        _______, _______, _______, _______, KC_VOLD,         KC_VOLU,           _______,        _______, _______, KC_MPRV, KC_MPLY, KC_MNXT,          _______,  _______,
        _______, _______, _______,          _______,         _______,           _______,        _______,                                     _______, _______,  _______
    ),
    [_LAYER2] = LAYOUT_all( /* Capslock Layer */
        MACRO_TOGGLE_TERMINAL, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
        _______, MACRO_DISCORD, A(KC_F4), MACRO_ENV, _______, _______, _______, _______, _______, _______, KC_PSCR, _______, _______,          _______,
